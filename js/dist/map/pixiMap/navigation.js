define(['./wpix', 'core/pubsub', 'core/util'], function (wpix, newPubSub, u) {
	...
	function init() {
		...
		p.dotPos = {
			get x() {
				return rect.x + rect.width - p.dotHalf;
			},
			get y() {
				return rect.y + rect.height - p.dotHalf;
			}
		};
		dot = p.dot;
		...
		p.navPos = new PIXI.Point(wpix.renderer.width - (p.panelWidth + p.panelOffset), 50);
		nav = p.nav;
	...
	function addDragDrop(el, name) {
		if (name === "rect") {
			el.on("mousedown", rectDown).on("touchstart", rectDown).on("mouseup", rectUp).on("mouseupoutside", rectUp).on("touchend", rectUp).on("touchendoutside", rectUp).on("mousemove", rectMove).on("touchmove", rectMove);
		} else if (name === "dot") {
			el.on("mousedown", dotDown).on("touchstart", dotDown).on("mouseup", dotUp).on("mouseupoutside", dotUp).on("touchend", dotUp).on("touchendoutside", dotUp).on("mousemove", dotMove).on("touchmove", dotMove);
		}
	}

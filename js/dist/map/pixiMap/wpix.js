define(["core/util", "core/pubsub"], function (u, newPubSub) {
  ...
  zoom(e); */
	...
	var pan = function () {
		...
		function add(el) {
			el.on("mousedown", down).on("touchstart", down).on("mouseup", up).on("mouseupoutside", up).on("touchend", up).on("touchendoutside", up).on("mousemove", move).on("touchmove", move);
		}
	...
	function create2pointLine(conf) {
		...
		function addEvents() {
			line.on("mousedown", down).on("touchstart", down).on("mouseup", up).on("mouseupoutside", up).on("touchend", up).on("touchendoutside", up)
			//	.on("mousemove", move)
			//	.on("touchmove", move)
			.on("mouseover", over).on("mouseout", out).on("rightup", rightup);
		}
		...
		function calcPoints() {
			...
			sX < eX && sY > eY) {
				// bottLeft to topRight
				sLeft = new PIXI.Point(sX - half, sY - half);
				sRight = new PIXI.Point(sX + half, sY + half);
				eRight = new PIXI.Point(eX + half, eY + half);
				eLeft = new PIXI.Point(eX - half, eY - half);
			} else if (sX === eX && (sY > eY || sY < eY)) {
				// vertical
	...
	function createBoxSpriteText(conf) {
		...
		function addEvents(el) {
			el.on("mousedown", down).on("touchstart", down).on("mouseup", up).on("mouseupoutside", up).on("touchend", up).on("touchendoutside", up).on("mousemove", move).on("touchmove", move).on("mouseover", over).on("mouseout", out).on('rightup', rightup);
		}
		function makeSprite() {

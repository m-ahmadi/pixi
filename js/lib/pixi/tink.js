...
var _createClass = (function () {
	...
	var fn = function (Constructor, protoProps, staticProps) {
		if (protoProps) {
...
var Tink = (function () {
  ...
  // into the `draggableSprites` array
	...
	_createClass(Tink, [{
		...
		value: function makePointer () {
			...
			var pointer = {
				...
				hitTestSprite: function hitTestSprite(sprite) {
					...
					if (!sprite.circular) { // Is the sprite rectangular?
		...
		/*
			Many of Tink's objects, like pointers, use collision detection using the sprites' global x and y positions.
			To make this easier, new `gx` and `gy` properties are added to sprites that reference Pixi sprites' `getGlobalPosition()` values.
		*/
	}, {
